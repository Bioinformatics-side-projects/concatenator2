...
def test_fasta_to_phylip():
    ...
    assert filecmp.cmp('testfasta.phy','Testing/TestFilesOutput/testfastaToPhy.phy',shallow=False)

def test_fasta_to_aln():
    f = fasta_converter("Testing/TestFilesInput/testfasta.fasta", "fasta", "aln", "no")

    assert filecmp.cmp('testfasta.aln','Testing/TestFilesOutput/testfastaToAln.aln',shallow=False)
